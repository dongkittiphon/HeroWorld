...
public class robot extends  Enemy {
    ...
    public robot(PlayScreen screen, float x, float y) {
        ...
        for (int i = 0; i < 2; i++) {
            frames.add(new TextureRegion(screen.getAtlas().findRegion("robot"),i*300,0,300,300));
        }
    ...
    public void update(float dt) {
        ...
        if(setToDestroy && !destroyed){
            ...
            destroyed = true;
            setRegion(new TextureRegion(screen.getAtlas().findRegion("robot"), 900,0,300,300));
            stateTime = 0;
        }
        else if(!destroyed) {
