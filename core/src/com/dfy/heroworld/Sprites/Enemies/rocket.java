...
public class rocket extends Enemy {
    ...
    public rocket(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        frames = new Array<TextureRegion>();
        for (int i = 0; i < 2; i++) {
            frames.add(new TextureRegion(screen.getAtlas().findRegion("rocket"),i*200,0,200,300));
        }
    ...
    public void update(float dt) {
        ...
        if(setToDestroy && !destroyed){
            ...
            destroyed = true;
            setRegion(new TextureRegion(screen.getAtlas().findRegion("rocket"), 600, 300, 200, 300));
            stateTime = 0;
        }
        else if(!destroyed) {
