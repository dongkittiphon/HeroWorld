...
public class shell extends Enemy {
    ...
    public shell(PlayScreen screen, float x, float y) {
        ...
        for (int i = 0; i < 2; i++) {
            frames.add(new TextureRegion(screen.getAtlas().findRegion("shell"),i*200,0,200,100));
        }
    ...
    public void update(float dt) {
        ...
        if(setToDestroy && !destroyed){
            ...
            destroyed = true;
            setRegion(new TextureRegion(screen.getAtlas().findRegion("shell"),600,100, 200, 100));
            stateTime = 0;
        }
        else if(!destroyed) {
