 ...
 */
  public class Butterfly extends  Enemy{
    ...
    public Butterfly(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        frames = new Array<TextureRegion>();
        for (int i = 0; i < 2; i++) {
            frames.add(new TextureRegion(screen.getAtlas().findRegion("Butterfly"),i*1565,513,130,150));
        }
    ...
    public void update(float dt) {
        ...
        if(setToDestroy && !destroyed){
            ...
            destroyed = true;
            setRegion(new TextureRegion(screen.getAtlas().findRegion("Butterfly"), 1825, 513,130,150));
            stateTime = 0;
        }
        else if(!destroyed) {
